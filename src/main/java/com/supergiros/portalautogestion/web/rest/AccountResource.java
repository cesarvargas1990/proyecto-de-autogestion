...
import com.supergiros.portalautogestion.service.dto.PasswordChangeDTO;
import com.supergiros.portalautogestion.web.rest.UserJWTController.JWTToken;
import com.supergiros.portalautogestion.web.rest.errors.*;
import com.supergiros.portalautogestion.web.rest.vm.KeyAndPasswordVM;
import com.supergiros.portalautogestion.web.rest.vm.ManagedUserVM;
import java.util.*;
...
public class AccountResource {
    ...
    public ResponseEntity<JWTToken> validateToken(@RequestBody String token) {
        Optional<User> user = userService.tokenAuthentication(token);

        if (!user.isPresent()) {
            throw new AccountResourceException("No user was found for this reset key");
        } else {
            String jwt = user.get().getActivationKey();
